...
bash+:version-check() {
	...
	if [[ ${cmd[*]} == bash ]]; then
		...
		out=$("${cmd[@]}") ||
			{
				echo "Failed to run '${cmd[*]}'" >&2
				exit 1
			}
		[[ $out =~ ([0-9]+\.[0-9]+(\.[0-9]+)?) ]] ||
			{
				echo "Can't determine version number from '${cmd[*]}'" >&2
				exit 1
			}
...
bash+:version-check bash 3.2 ||
	{
		echo "The 'bashplus' library requires 'Bash 3.2+'." >&2
		exit 1
	}
...
bash+:use() {
	local library_name=${1:?bash+:use requires library name}
	shift
	local library_path=
	library_path=$(bash+:findlib "$library_name") || true
	[[ $library_path ]] ||
